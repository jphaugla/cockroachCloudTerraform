...
# ──────────────────────────────────────────────────────────────────────────────
# 1) GCP provider aliases for each region
# ──────────────────────────────────────────────────────────────────────────────
# at top of file, make sure you have:
provider "google" {
  alias  = "region0"
  project = var.project_id
  region = var.gcp_region_list[0]
}

provider "google" {
  alias  = "region1"
  project = var.project_id
  region = local.region_count > 1 ? var.gcp_region_list[1] : local.default_region
}

provider "google" {
  alias  = "region2"
  project = var.project_id
  region = local.region_count > 2 ? var.gcp_region_list[2] : local.default_region
}
...
module "crdb-region-0" {
  ...
  psc_producer_project_id = var.gcp_account_id
  providers = {
    google = google.region0
  }
  ...
  # GCP & Networking
  virtual_network_location       = var.gcp_region_list[0]
  ...
  instances_inventory_file           = "../../terraform-gcp-app/inventory-${var.gcp_region_list[0]}"
  playbook_instances_inventory_file  = "../terraform-gcp-app/inventory-${var.gcp_region_list[0]}"
...
module "crdb-region-1" {
  ...
  virtual_network_location       = var.gcp_region_list[1]
  ...
  playbook_working_directory         = "../../ansible"
  instances_inventory_file           = "../../terraform-gcp-app/inventory-${var.gcp_region_list[1]}"
  playbook_instances_inventory_file  = "../terraform-gcp-app/inventory-${var.gcp_region_list[1]}"
  instances_inventory_directory      = "temp"
  inventory_template_file            = "templates/inventory.tpl"
}
...
module "crdb-region-2" {
  ...
  virtual_network_location       = var.gcp_region_list[2]
  ...
  playbook_working_directory         = "../../ansible"
  instances_inventory_file           = "../../terraform-gcp-app/inventory-${var.gcp_region_list[2]}"
  playbook_instances_inventory_file  = "../terraform-gcp-app/inventory-${var.gcp_region_list[2]}"
  instances_inventory_directory      = "temp"
  inventory_template_file            = "templates/inventory.tpl"
}
